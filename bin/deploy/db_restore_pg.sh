#!/usr/bin/env bash
## =========================================================================
#   Reset database using attached dump.
#
...
echo "Extract local dump ${FILE_TGZ}..."
tar -C ${DIR_DATA} -zxf ${FILE_TGZ}
if [ -f "${FILE_SQL}" ]; then
    echo "Dump is extracted into ${FILE_SQL}."
else
    echo "There is no expected file ${FILE_SQL}. Aborting..."
    cd ${DIR_CUR}
    exit 250
fi
...
echo "Restore database '${DB_NAME}' from dump '${FILE_SQL}'..."
${PSQL} < ${FILE_SQL} > /dev/null
ERR=$?
if [ ${ERR} -ne 0 ]; then
    echo "Cannot restore DB '${DB_NAME}'. Aborting..."
    exit ${ERR}
fi

rm ${FILE_SQL}
echo "Plain SQL dump '${FILE_SQL}' is removed."
